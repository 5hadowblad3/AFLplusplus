...
class Miscs:
    ...
    @beartype    
    @classmethod
    def get_deg(cls, nvs: int, nts: int, max_deg: int = 7):
        """
    ...
    def remove_ugly(cls, ps):
        ...
        def is_nice_coef(c: Union[int, float]):
            return abs(c) <= settings.UGLY_FACTOR or c % 10 == 0 or c % 5 == 0

        def is_nice_eqt(eqt: Union[sympy.Expr, sympy.Rel]):
...
class MP:
    ...
    @classmethod
    def wprocess(cls, f, mytasks, myQ):
        try:
            rs = f(mytasks)
    ...
    @beartype            
    @classmethod
    def run_mp(cls, taskname: str, tasks, f, DO_MP: bool):
        """
